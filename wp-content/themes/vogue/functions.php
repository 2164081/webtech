 ...
 * @package Vogue
 */
define( 'VOGUE_THEME_VERSION' , '1.4.34' );
 ...
 */
function vogue_flash_notice() {
	global $current_user;
	$user_id = $current_user->ID;
	
	if ( !get_user_meta($user_id, 'vogue_flash_notice_ignore') ) {
		echo '<div class="updated notice vogue-notice-layouts"><p>'. __( 'Vogue Premium is now on a <a href="https://kairaweb.com/go/vogue/" target="_blank">Flash Sale</a> for only $15', 'vogue' ) .' <a href="?vogue-flash-notice-ignore" class="vogue-noticemiss">Dismiss</a></p></div>';
	}
}
add_action('admin_notices', 'vogue_flash_notice');

function vogue_flash_notice_ignore() {
	global $current_user;
	
	$user_id = $current_user->ID;
	if (isset($_GET['vogue-flash-notice-ignore'])) {
		add_user_meta($user_id, 'vogue_flash_notice_ignore', 'true', true);
	}
}
add_action('admin_init', 'vogue_flash_notice_ignore');
